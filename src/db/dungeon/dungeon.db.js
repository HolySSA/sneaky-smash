...
export const createDungeon = async (stageList) => {
  const result = await handleDbQuery(dbPool.query.bind(dbPool), [
    SQL_QUERIES.CREATE_DUNGEON, stageList
  ]);
...
export const findDungeonById = async (id) => {
  const rows = await handleDbQuery(dbPool.query.bind(dbPool), [SQL_QUERIES.FIND_DUNGEON_BY_ID, id]);
  if (rows.length === 0) {
...
export const findAllDungeons = async () => {
  const rows = await handleDbQuery(dbPool.query.bind(dbPool), [SQL_QUERIES.FIND_ALL_DUNGEONS]);
