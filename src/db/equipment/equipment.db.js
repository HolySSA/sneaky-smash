...
export const createEquipment = async (equipment) => {
  const { gold, ATK, DEF, MaxHp, CriticalDamageRate, CriticalProbability, CurHp, MoveSpeed } = equipment;
...
export const getEquipmentById = async (id) => {
  const row = await handleDbQuery(dbPool.query.bind(dbPool), [SQL_QUERIES.GET_EQUIPMENT_BY_ID, id]);
  if (!row) {
...
export const getAllEquipments = async () => {
  const rows = await handleDbQuery(dbPool.query.bind(dbPool), [SQL_QUERIES.GET_ALL_EQUIPMENTS], true);  // isArray = true
