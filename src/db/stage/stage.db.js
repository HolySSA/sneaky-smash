...
export const findStageById = async (id) => {
  ...
  return stage.rows; // 이미 카멜케이스로 변환된 결과 반환
};

// 모든 스테이지 조회
export const findAllStages = async () => {
  const stages = await handleDbQuery(
    dbPool.query.bind(dbPool),
    [SQL_QUERIES.FIND_ALL_STAGES],
    true,
  ); // 여러 결과 반환
  return stages.rows; // 이미 카멜케이스로 변환된 결과 반환
};

// 스테이지 수정
export const updateStage = async (id, MonsterID, MonsterCount) => {
  const result = await handleDbQuery(dbPool.query.bind(dbPool), [
    SQL_QUERIES.UPDATE_STAGE,
    MonsterID,
    MonsterCount,
    id,
  ]);
