...
const onData = (socket) => async (data) => {
  ...
  while (socket.buffer.length >= totalHeaderLength) {
    ...
    if (socket.buffer.length >= packetLength) {
      const packet = socket.buffer.subarray(totalHeaderLength, packetLength + totalHeaderLength);
      socket.buffer = socket.buffer.subarray(packetLength + totalHeaderLength);
